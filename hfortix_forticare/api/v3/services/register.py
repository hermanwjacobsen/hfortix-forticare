...
Example Usage:
    ...
    >>> response = fcc.api.services.register.post(contract_number=..., account_id=...)
    >>>
...
Required permission: ReadWrite/Admin
        ...
        Args:
            contract_number: Contract number to register (required)
            account_id: Service's account ID. Optional for API user with Local scope, mandatory for Org scope. (optional)
            description: Set product description during registration process (optional)
            additional_info: Store extra info for certain product registration, for example system ID, IP address etc. (optional)
            is_government: Whether the product will be used for government or not (optional)
        ...
        return ServiceRegisterResponseWithAssetDetails(
            data=envelope["data"],
